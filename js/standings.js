// standings.js — standings view: all-time career standings & results.
// Wrapped in an IIFE so its render/fmtNum/init don't collide with
// record_book.js's identically-named functions now that both views
// live on the same page (classic <script> tags share one global scope
// for function declarations — two `function render(){}` on the same
// page would silently overwrite each other without this).
...
const StandingsView = (() => {
  ...
  function fmtNameBadges(m) {
    ...
    return badges
      ? ` <span class="name-badges" title="${m.titles || 0} title(s), ${m.last_place_finishes || 0} last-place finish(es)">${badges}</span>`
      : '';
  }
